...
function Electronics(props) {
    ...
    var electronicsUrl = "https://fakestoreapi.com/products/category/electronics";

    const [electronicsData, setElectronicsData] = useState([]);

    useEffect(() => {
        axios.get(electronicsUrl)
            .then(response => setElectronicsData(response.data))
    }, [electronicsUrl]);

    const imageArrayUpdatedElectronicsData = electronicsData.map((item) => (
        ...
        <React.Fragment>
            <Container maxWidth="xl" sx={{ bgcolor: '#faf5f5', position: 'sticky', top: 88, zIndex: 998 }}>
                <Stack direction="row" spacing={2} sx={{ justifyContent: 'space-between', alignItems: 'center', py: 3, mb: 2 }}>
                    <Typography sx={{ fontSize: 30, fontWeight: 500 }}>Electronics</Typography>
                    <Typography>
            ...
            <Stack my={5} direction='row' sx={{ justifyContent: 'center', flexWrap: 'wrap' }}>
                {
                    imageArrayUpdatedElectronicsData.map((item) => (
